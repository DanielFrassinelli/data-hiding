...
threshold = 3.378135; % now is dynamic --> we will fix it during the competition
...
for i = 1:dct_list_size
    ...
    for r = 1:3
        for c = 1:3
            for k = 1:similar_size
                temp(k) = dct.original(block(i).sInd(k) , r , c);
            end            
            z(p) = var(temp);
            if (c == 1 && r == 1) 
                z(p) = inf;
    ...
    block(i).row = a;
    block(i).col = b;
    block(i).param = computeBlockParameter(dct.original(i,:,:), block(i).row, block(i).col); 
end
...
fprintf('Calcola la soglia tra le 3 immagine originali e poi salvala');
if (sim >= threshold)
    a = 1;
else
    a = 0;
